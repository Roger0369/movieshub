...
describe('MovieCatalogComponent', () => {
  ...
  it('should return sanitized movie key', () => {
    const result = component.getKeyFromMovieName('Spider-Man: No Way Home!');
    expect(result).toBe('spidermannowayhome');
  });

  it('should navigate to movie details with correct key', () => {
    const movie = mockMovies[0];
    const expectedKey = component.getKeyFromMovieName(movie.name);

    component.seeDetails(movie);

    expect(router.navigate).toHaveBeenCalledWith(['/pelicula', expectedKey]);
  });
});
